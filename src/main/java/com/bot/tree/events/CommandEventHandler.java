...
public class CommandEventHandler {
	...
	static final String vips[] = {"maerklig", "goat_sam", "treenarchy", "runecarsnowpen", "paleontologistsva", "iv4nhoe", "cryptic_dayv"};
    ...
    public void onChannelMessage(ChannelMessageEvent event) {
		if(LocalDateTime.now().minusSeconds(30).isAfter(lastMessage))
		{
			...
			if(msg.contains("pripps")) {
				...
				event.getTwitchChat().sendMessage(event.getChannel().getName(), "🤖 You've seen the awesome work we do. Please consider giving us a 5 star review on Google. https://bit.ly/arborisbetyg");
			}
			...
			else if(msg.startsWith("!fix") && Arrays.asList(vips).contains(event.getUser().getName().toLowerCase())) {
				event.getTwitchChat().sendMessage(event.getChannel().getName(), "!fix Fixing the stream because of lagging input sources. Just wait a few seconds and everything will be fine.");
			}
		}
	}
